...
vec3 normalDirection = normalize(worldNorm); //kosinový úhel výrazu bude v rozmezí od -1 do 1
...
void main (void)
{
	...
	for (int i = 0; i < numLights; i++) {
		...
		if(light.type == 0){
			finalColor += (diffuse + specular) * vec4(objectColor, 1.0);
		}
		else if(light.type == 1){
			finalColor += attenuation * (diffuse + specular) * vec4(objectColor, 1.0);
		}
		else if((light.type == 2 || light.type == 3) && light.isOn == 1){
			finalColor += attenuation * intensity * (diffuse + specular) * vec4(objectColor, 1.0);
		}
	}
